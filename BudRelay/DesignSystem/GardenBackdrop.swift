...
struct GardenBackdrop: View {
    enum Scene {
        case home, garden, greenhouse, market, daily
    }
            ...
            ZStack {
                ...
                ForEach(blobs) { blob in
                    Circle()
                        ...
                        .position(x: blob.x * proxy.size.width, y: blob.y * proxy.size.height)
                        .blur(radius: blob.radius * 0.55)
                }
                edgeFlowers(in: proxy.size)
